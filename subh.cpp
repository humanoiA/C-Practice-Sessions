...
main()
{
	...
	for(i=0;i<3;i++)
	{
		for(j=0;j<3;j++)
		{
			...
			{
			C[i][j]=A[i][j-1]*B[i][j]+A[i][j+1]*B[i][j]+A[i][j+1]*B[i+2][j];	
			printf("%d\t",C[i][j]);
		    }
		    else if(i==0&&j==2)
		    {
		    	C[i][j]=A[i][j+2]*B[i][j+2]+A[i][j+1]*B[i+1][j]+A[i][j+2]*B[i+2][j+2];
		    	printf("%d\n",C[i][j]);
			}
			else if(i==1&&j==0)
			{
				C[i][j]=A[i][j]*B[i][j+1]+A[i][j+1]*B[i][j]+A[i][j+2]*B[i+1][j+2];
				printf("%d\t",C[i][j]);
			}
			else if(i==1&&j==1)
			{
			...
			{
				C[i][j]=A[i][j]*B[i-2][j]+A[i][j+1]*B[i-1][j]+A[i][j+2]*B[i][j+1];
				printf("%d\t",C[i][j]);
			}
			else if(i==2&&j==1)
			{
				C[i][j]=A[i][j-1]*B[i-2][j]+A[i][j]*B[i-1][j]+A[i][j+1]*B[i-1][j];
				printf("%d\t",C[i][j]);
			}
			else
			{
